...
/***/ function(module, exports, __webpack_require__) {
	...
	function init () {
		bindEvent();
		// var company_id = $(".searchPannel .company_id").val();
		// $(".searchPannel .department_id,.searchPannel .position_id,.searchPannel .grade_id").val(-1);
		
		// cascadeAjax($(".searchPannel .department_id"),company_id);
	}
	...
	function bindEvent(){
		...
		$(".editBtn").on("click",function(){
			...
			$("#myModal .company_id").val($tr.find("td").eq(0).attr("data-value"));
			$("#myModal .department_id").val($tr.find("td").eq(1).attr("data-value"));
			$("#myModal .position_id").val($tr.find("td").eq(2).attr("data-value"));
			if( !$tr.find("td").eq(3).attr("data-value") ) return;
			$("#myModal .grade_id").val($tr.find("td").eq(3).attr("data-value"));
		});
			// 日历
		...
		$(".saveBtn").on("click",function(){
			var company_id = $("#myModal .company_id").val();
			var department_id = $("#myModal .department_id").val();
			var position_id = $("#myModal .position_id").val();
			var grade_id = $("#myModal .grade_id").val();
			var company = "";
			var department = "";
			var position = "";
			var grade  = "";
			...
			if(company_id!=null){
				company=$("#myModal .company_id").find("option:selected").text();
			}else{
				modalAlert("请选择公司！");
				return;
			}
			if(department_id!=null){
				department=$("#myModal .department_id").find("option:selected").text();
			}else{
				modalAlert("请选择部门！");
				return;
			}
			if(position_id!=null){
				position=$("#myModal .position_id").find("option:selected").text();
			}else{
				modalAlert("请选择岗位！");
				return;
			}
			if(grade_id!='-1'){
				grade=$("#myModal .grade_id").find("option:selected").text();
			}else{
				// modalAlert("请选择职级！");
				// return;
			}
			...
			$.ajax({
		        url: host + '/ehradmin/structure/add-struct',
		        type: 'post',
		        dataType: 'json',
		        data: {company_id:company_id,department_id:department_id,position_id:position_id,grade_id:grade_id,company:company,department:department,position:position,grade:grade}
		    })
		    .done(function(data) {
		       if(data.success){
		...
		$(".updateBtn").on("click",function(){
			...
			$.ajax({
		        url: host + '/ehradmin/structure/update-struct',
		        type: 'post',
		        dataType: 'json',
		        data: {id:id,company_id:company_id,department_id:department_id,position_id:position_id,grade_id:grade_id,company:company,department:department,position:position,grade:grade}
		    })
		    .done(function(data) {
		       if(data.success){
		          window.location.reload();
		...
		//搜索栏
		$(".searchPannel .company_id").on("change",function(){
				var company_id = $(".searchPannel .company_id").val();
				$(".searchPannel .department_id,.searchPannel .position_id,.searchPannel .grade_id").val(-1);
				
				cascadeAjax($(".searchPannel .department_id"),company_id);
		});
		$(".searchPannel .department_id").on("change",function(){
				var company_id = $(".searchPannel .company_id").val();
				var department_id = $(".searchPannel .department_id").val();
				$(".searchPannel .position_id,.searchPannel .grade_id").val(-1);
				
				cascadeAjax($(".searchPannel .position_id"),company_id,department_id);
		});
		$(".searchPannel .position_id").on("change",function(){
				var company_id = $(".searchPannel .company_id").val();
				var department_id = $(".searchPannel .department_id").val();
				var position_id = $(".searchPannel .position_id").val();

				$(".searchPannel .grade_id").val(-1);
				cascadeAjax($(".searchPannel .grade_id"),company_id,department_id,position_id);
				
		});

		//新增&编辑
		// $("#myModal .company_id").on("change",function(){
		// 		var company_id = $("#myModal .company_id").val();
		// 		$("#myModal .department_id,#myModal .position_id,#myModal .grade_id").val(-1);
				
		// 		cascadeAjax($("#myModal .department_id"),company_id);
		// });
		// $("#myModal .department_id").on("change",function(){
		// 		var company_id = $("#myModal .company_id").val();
		// 		var department_id = $("#myModal .department_id").val();
		// 		$("#myModal .position_id,#myModal .grade_id").val(-1);
				
		// 		cascadeAjax($("#myModal .position_id"),company_id,department_id);
		// });
		// $("#myModal .position_id").on("change",function(){
		// 		var company_id = $("#myModal .company_id").val();
		// 		var department_id = $("#myModal .department_id").val();
		// 		var position_id = $("#myModal .position_id").val();

		// 		$("#myModal .grade_id").val(-1);
		// 		cascadeAjax($("#myModal .grade_id"),company_id,department_id,position_id);
				
		// });

	}

	function cascadeAjax($select,company_id,department_id,position_id){
		$.ajax({
	        url: host + '/ehradmin/structure/cascade',
	        type: 'post',
	        dataType: 'json',
	        data: {company_id:company_id,department_id:department_id,position_id:position_id}
	    })
	    .done(function(data) {
	       if(data.success){
