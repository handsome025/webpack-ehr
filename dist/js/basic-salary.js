...
/***/ function(module, exports, __webpack_require__) {
	...
	var queryName = __webpack_require__(16); //查询姓名
	...
	function init () {
		...
		// 	modalTitle = "（礼天下）" 
		// }
		// $(".basic-salary .panel-title").text("底薪管理"+modalTitle)
		bindEvent();
		index({page:1});
		// initDate();
	}
	...
	function index(data){
		...
		.always(function(){
	    	...
	    	if(result.length!=0){
				...
				$('.basic-salary tbody').html(_html);
				pagingEvent(res.result.current_page,res.result.total_page);//翻页
				page();//翻页禁用
				
				// sortTable()
			}else{  
				$('.basic-salary tbody').html('<tr><td colspan="7">未搜索到符合条件的数据</td></tr>');
			}
			...
			if(btnPer.create){
				$(".basic-salary .addbtn").removeClass('none');
			}if(btnPer.edit){
				$(".basic-salary .operation").removeClass('none');
			}if(btnPer.import){
				$(".basic-salary .intoBtn").removeClass('none');
			}
		},function(res){
	...
	function bindEvent(){
		...
		$(".saveBtn").on("click",function(){
			...
			if($.trim(person_id)==""||$.trim(start_day)==""||$.trim(basic_salary)==""){
				return;
			}
			var data = {
				person_id:person_id,
		...
		$(document).on("click",".editbtn",function(){
			...
			$("#myModal .start_day").val(tr.find("td").eq(5).text());
			$("#myModal .end_day").val(tr.find("td").eq(6).text());
		});
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function($) {function queryName(dom) {
		var enable
