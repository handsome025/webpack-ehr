...
/***/ function(module, exports, __webpack_require__) {
	    	...
	    	if(result.length!=0){
				for(var i=0; i<result.length; i++){
	               _html +='<tr id='+result[i]._id+'>'
			          ...
			          +'<td>'+result[i].punch_time_should_be_afternoon+'</td>'
			          +'<td>'+result[i].actual_punch_time_in_afternoon+'</td>'
			          +'<td>早退'+result[i].leave_early_minutes+'分钟</td>'
	            }
				$('.early-leave tbody').html(_html);

				pagingEvent(data.p,Math.ceil(res.result.total/10));//翻页
				page();//翻页禁用
			}else{  
				$('.early-leave tbody').html('<tr><td colspan="7">未搜索到符合条件的数据</td></tr>');
			}
	    },function(res){
	...
	function bindEvent(){
		...
		$(".exportbtn").on("click",function(){
			...
			var attendance_date = $("#attendance_date").val();
			window.location.href = origin + '/attendance/offearly/export?person_name='+person_name+'&company_name='+company_name+'&attendance_date='+attendance_date;
		})
