...
/***/ function(module, exports, __webpack_require__) {
	...
	function init(){
		stratumAjax($("#myModal .company_id"),1);
		allotMapAjax($("#myModal .role_id"));
		bindEvent();
	}
	...
	function bindEvent(){
		$(".addBtn").on("click",function(){
			...
			$(".saveBtn").attr("data-cid",$(this).attr("data-cid"));
		});
		...
		$(".saveBtn").on("click",function(){
			...
			var dept_id = $("#myModal .department_id").val();
			var post_id = $("#myModal .position_id").val();
			var role_id = $("#myModal .role_id").val();
			var is_enabled = $("#myModal .status_id").val();
			var company_name = "";
			var dept_name = "";
			var post_name = "";
			var role_name = "";
			...
			if(company_id!=null){
				company_name=$("#myModal .company_id").find("option:selected").text();
			}else{
				modalAlert("请选择公司！");
				return;
			}
			if(dept_id!=null){
				dept_name=$("#myModal .department_id").find("option:selected").text();
			}else{
				modalAlert("请选择部门！");
				return;
			}
			if(post_id!=null){
				post_name=$("#myModal .position_id").find("option:selected").text();
			}else{
				modalAlert("请选择岗位！");
				return;
			}
			if(role_id!=null){
				role_name=$("#myModal .role_id").find("option:selected").text();
			}else{
				modalAlert("请选择角色！");
				return;
			}
			...
			$.ajax({
		        url: host + '/sys/role/allot-save',
		        type: 'post',
		        dataType: 'json',
		        data: {company_id:company_id,dept_id:dept_id,post_id:post_id,role_id:role_id,company_name:company_name,dept_name:dept_name,post_name:post_name,role_name:role_name,is_enabled:is_enabled}
		    })
		    .done(function(data) {
		       if(data.success){
	...
	function cascadeAjax($select,company_id,department_id,position_id){
	    ...
	    .done(function(data) {
	       if(data.success){
	          ...
	          var html = '<option value="-1" disabled selected>请选择</option>';
	          for(var i in result){
	...
	//获取公司部门岗位
	function stratumAjax($select,type){
		$.ajax({
	        url: host + '/ehradmin/structure/get-stratum',
	        type: 'get',
	        dataType: 'json',
	        data: {type:type}
	    })
	...
	//获取角色
	function allotMapAjax($select){
		$.ajax({
	        url: host + '/sys/role/allot-map',
	        type: 'get',
	        dataType: 'json',
	        data: {}
	    })
	    .done(function(data) {
	       if(data.success){
	          var result = data.result.role_list;
	          var html = '<option value="-1" disabled selected>请选择</option>';
	          for(var i in result){
	...
	function getRoleInfo(id){
		var company_id,dept_id,post_id,role_id,is_enabled;
		var promiseA = $.ajax({
	        url: host + '/sys/role/allot-edit',
	        type: 'get',
	        dataType: 'json',
	        data: {id:id}
	    });
		...
		var promiseB = promiseA.then(function(data) {
	       if(data.success){
			...
			//  return  cascadeAjax($("#myModal .department_id"),company_id);
	    ...
	    var promiseC = promiseB.then(function(data){
		        ...
		        $("#myModal .department_id").html(html);
	    		return $.ajax({
			        url: host + '/ehradmin/structure/cascade',
			        type: 'post',
			        dataType: 'json',
			        data: {company_id:company_id,department_id:dept_id}
			    })
	    		// return cascadeAjax($("#myModal .position_id"),company_id,department_id);
	    });
	    ...
	    promiseC.then(function(data){
	        ...
	        $("#myModal .position_id").html(html);

	    	$("#myModal .company_id").val(company_id);
			$("#myModal .department_id").val(dept_id);
			$("#myModal .position_id").val(post_id);
			$("#myModal .role_id").val(role_id);
			$("#myModal .status_id").val(is_enabled);
