...
/***/ function(module, exports, __webpack_require__) {
	...
	$(".uploadShow").on('click',function(){
		$(".upload-main").html('');
		var cardcode = $(this).attr('cardcode');
		$(this).next().html('<iframe src="image-upload/index.html?cardcode='+cardcode+'&person_id='+person_id+'" name="main" frameborder="0" width="100%" height="250"></iframe>');
		$(".list-last").removeClass('append-html');
		$(this).parents('.panel-body').find('.list-last').addClass('append-html');
	});
	...
	// 删除证书
	$(document).on('click',".delete-cer",function(){
		var $this = $(this);
		var cer_id = $this.attr('_id');
		$this.parent().addClass('deleteClass');
		if( !ajaxLock ) return; ajaxLock = false; $this.addClass('disabled');
		$.ajax({
			url: windowOrigin+'/ehradmin/archive/delete-cer	',
			type: 'POST',
			dataType: 'json',
			data: { person_id:person_id, cer_id:cer_id },
		})
		.done(function(data) {
			console.log(data);
			if( data.success ){
	...
	listLast();
	function listLast(){
		$(".list-last:eq(0) > div").each(function(i) {
			eachStyle($(this),i);
		});
		$(".list-last:eq(1) > div").each(function(i) {
			eachStyle($(this),i);
		});
		$(".list-last:eq(2) > div").each(function(i) {
			eachStyle($(this),i);
		}); 
		$(".list-last:eq(3) > div").each(function(i) {
			eachStyle($(this),i);
		}); 
		$(".list-last:eq(4) > div").each(function(i) {
			eachStyle($(this),i);
		}); 
	}
